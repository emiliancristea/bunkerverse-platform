...
async fn get_model_info(State(state): State<AppState>) -> Result<Json<ModelInfoResponse>, StatusCode> {
    let engine = state.nar_engine.read().await;
...
async fn generate_ai_response(
    ...
    if let Some(ref engine) = *engine {
        ...
        match engine.generate(request) {
            Ok(response) => {
                let model_info = engine.get_model_info().unwrap_or_else(|_| serde_json::json!({}));
                
                info!("✅ AI generation completed: {} tokens in {}ms", 
...
async fn validate_engine(State(state): State<AppState>) -> Result<Json<serde_json::Value>, StatusCode> {
    let engine = state.nar_engine.read().await;
...
async fn main() -> anyhow::Result<()> {
    ...
    let config = NarConfig {
        model_path: std::env::var("MODEL_PATH").unwrap_or_else(|_| "models/gemma-3-1b.gguf".to_string()),
        context_size: 2048,
    ...
    info!("🔧 Configuration loaded: {}", serde_json::to_string(&config)?);
