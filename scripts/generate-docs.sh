...
mkdir -p "$OUTPUT_DIR"/{protobuf,openapi,markdown,static}

# Generate documentation using buf
echo "🔧 Generating Protocol Buffer documentation..."
buf generate --template buf.gen.docs.yaml

# Create custom HTML template with BUNKERVERSE branding
cat > "$OUTPUT_DIR/static/custom.css" << 'EOF'
:root {
...
<body>
    ...
    <div style="padding: 40px;">
        ...
        <footer style="margin-top: 50px; padding-top: 20px; border-top: 1px solid rgba(255,255,255,0.1); text-align: center; color: #888;">
            <p>Generated on $(date) | Version $VERSION | BUNKERVERSE Platform</p>
        </footer>
    </div>
</body>
</html>
...
echo "📊 Creating aggregated OpenAPI specification..."
cat > "$OUTPUT_DIR/openapi/bunkerverse-api-$VERSION.yaml" << EOF
openapi: 3.0.3
info:
...
echo "🌐 OpenAPI spec available at: $OUTPUT_DIR/openapi/bunkerverse-api-$VERSION.yaml"